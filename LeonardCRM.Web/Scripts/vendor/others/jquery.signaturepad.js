...
function SignaturePad (selector, options) {
  ...
  function drawIt () {
    ...
    $(settings.typeSignLabel, context).hide()
    $(settings.typeSignInput, context).hide()
    $(settings.typeSignInput, context).val('')
    $(settings.typed, context).empty()
  ...
  function typeIt () {
    ...
    $(settings.typeSignLabel, context).show()
    $(settings.typeSignInput, context).show()
  ...
  function onFormError (errors, context, settings) {
    ...
    if (errors.drawInvalid) {
    	if ($(settings.typeIt, context).hasClass("current")) {
    		context.prepend(['<p class="', settings.errorClass, '">', settings.errorMessageTypeSign, '</p>'].join(''));
    	}
    	else {
  ...
  function validateForm () {
    ...
    if (settings.drawOnly && output.length < 1) { //only draw mode
      errors.drawInvalid = true
      valid = false
    }
    else if (!settings.drawOnly) { //both mode
    	var typedSignEle = $(settings.typeIt, context);
    	if ((typedSignEle.hasClass("current") && $(settings.typeSignInput, context).val() == "") || //not type a siganture
			(!typedSignEle.hasClass("current") && output.length < 1)) //not draw a signature
    	{
    		errors.drawInvalid = true
    		valid = false
    	}
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
          if (!settings.drawOnly) {
              var syncWithTypeSignInput = function (value, eventName) {
                  $(settings.typeSignInput, context).val(value);
                  $(settings.typeSignInput, context).trigger(eventName);
              };

            $(settings.name, context).bind('keyup.signaturepad', function () {
                syncWithTypeSignInput($(settings.name, context).val(), 'keyup.signaturepad');
            });
            ...
            $(settings.name, context).bind('blur.signaturepad', function () {
      		...
      		$(settings.typeSignInput, context).bind('keyup.signaturepad', function () {
      			type($(this).val())
      		});

      		$(settings.typeSignInput, context).bind('blur.signaturepad', function () {
      			type($(this).val())
      		});
  ...
  $.extend(self, {
     ...
     * @return {String}
     */
    , getSignatureImage : function (ipClient, printName) {
      ...
      //add addition text     
      tmpContext.font = "15px Comic Sans MS";
      tmpContext.fillStyle = "red";
      tmpContext.textAlign = "left";
      tmpContext.fillText("Captured " + ipClient + " " + (new Date()).toLocaleString(), 0, 15);
      tmpContext.stroke();

      //add signature line
      tmpContext.beginPath();
      tmpContext.moveTo(10, 85);
      tmpContext.strokeStyle = 'gray';
      tmpContext.lineTo(340, 85);
      ...
      //add print name
      if (typeof printName != "undefined" && printName != null && printName != '') {
          tmpContext.font = "15px Comic Sans MS";
          tmpContext.fillStyle = "black";
          tmpContext.textAlign = "left";
          tmpContext.fillText(printName, 10, 100);
      }
	  
      if ($(settings.typeIt, context).hasClass("current")) {
      	var typedSign = $(settings.typed, context).text();

      	tmpContext.font = "normal 67px 'Herr Von Muellerhoff', cursive";
      	tmpContext.fillStyle = "#145394";
      	tmpContext.textAlign = "left";
      	tmpContext.fillText(typedSign, 10, 80, tmpCanvas.width - 15);
      }
      else {
...
$.fn.signaturePad.defaults = {
  ...
  , typeSignInput: '.typeSignInput'
  , typeSignLabel: '.typeSignLabel'
  , errorMessageTypeSign : 'Please type this signature' // The error message displayed when drawOnly and no signature is drawn
}
