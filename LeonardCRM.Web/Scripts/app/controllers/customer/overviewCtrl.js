(function (ng) {
    ...
    angular.module("LeonardCRM").controller("customerOverviewCtrl", ctrl);

    ctrl.$inject = ["$scope", "$window", "salesCustomerService", "registryService", "appService"];

    function ctrl($scope, $window, salesCustomerService, registryService, appService) {

        $scope.dateFormat = registryService.siteSettings.DATE_FORMAT + ' ' + registryService.siteSettings.TIME_FORMAT;
        $scope.defaultView = appService.getDefaultView('customer');
        ...
        chart1.options = {
            ...
            "vAxis": {
                "title": $scope.languages.SALES_CUSTOMER.OVERVIEW_CHART_Y_TITLE,
                "format": '#'
            },
            "hAxis": {
                "title": $scope.languages.SALES_CUSTOMER.OVERVIEW_CHART_X_TITLE,
        ...
        function loadTopItems() {
            salesCustomerService.getRecentlyAddedClients($scope.onlyMe).then(function (data) {
        ...
        function loadReport() {
            ...
            loadTopItems();
            salesCustomerService.getChartData($scope.onlyMe).then(function (data) {
        ...
        $scope.openCustomerDetail = function (defaultView, id) {
            var link = '#/customer/view/' + defaultView + '/' + id;
