(function (ng) {
    ...
    angular.module("LeonardCRM").controller("InvoiceOverviewCtrl", ctrl);

    ctrl.$inject = ["$scope", "$http", "$route", "$location", "$routeParams", "$window", "appService", "roleService", "registryService", "salesInvoiceService"];

    function ctrl($scope, $http, $route, $location, $routeParams, $window, appService, roleService, registryService, salesInvoiceService) {
        ...
        $scope.onlyMe = true;
        $scope.myOptions = [{ Text: $scope.languages.REPORT.RECENTLY_ADDED, Value: 0 },
        ...
        chart1.data = {
            "cols": [
                ...
                { id: "total", label: $scope.languages.INVOICES.OVERVIEW_CHART_Y_TITLE, type: "number" }
            ],
            "rows": [
            ]
        };
        ...
        chart1.options = {
            ...
            "vAxis": {
                "title": $scope.languages.INVOICES.OVERVIEW_CHART_Y_TITLE,
                "format": '#'
            },
            "hAxis": {
                "title": $scope.languages.INVOICES.OVERVIEW_CHART_X_TITLE,
        ...
        function loadTopItems() {
            salesInvoiceService.getRecentlyAddedInvoices($scope.onlyMe).then(function (data) {
        ...
        function loadReport() {
            ...
            loadTopItems();
            salesInvoiceService.getChartData($scope.onlyMe).then(function (data) {
        ...
        $scope.openInvoiceDetail = function (defaultView, id) {
            var link = '#/invoice/view/' + defaultView + '/' + id;
