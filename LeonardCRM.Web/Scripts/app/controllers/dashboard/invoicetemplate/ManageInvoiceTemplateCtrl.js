(function() {
    ...
    angular.module("LeonardCRM").controller("ManageInvoiceTemplateCtrl", ctrl);

    ctrl.$inject = ['$scope', '$http', '$location', 'InvTemplateService', 'filterService', 'salesInvoiceService', 'salesOrderService', 'registryService', 'toolbarService', 'viewService'];

    function ctrl($scope, $http, $location, InvTemplateService, filterService, salesInvoiceService, salesOrderService, registryService, toolbarService, viewService) {

            $scope.pageInfo = {};

            //---------START DELETE SALES INVOICES TEMPLATE-----------------
            $scope.DeleteSalesInvTemplate = function() {
                InvTemplateService.DeleteInvTemplate($scope.EditValues)
            ...
            $scope.$on('addEvent', function(e) {
                ...
                $location.path('/dashboard/invoice_templates/add');
            });

            $scope.$on('deleteEvent', function(e) {
                $scope.SetConfirmMsg($scope.languages.INVTEMPLATE.DELETE_MSG, 'invoicetemplate');
            });

            $scope.$on('yesEvent', function(event, args) {
                if (args == 'invoicetemplate') {
                    $scope.DeleteSalesInvTemplate();
                }
            });

            $scope.$on('rowClickEvent', function(event, args) {
                toolbarService.NeedSaveCommand(true);
                $location.path('/dashboard/invoice_templates/' + args.RowId + '/view/' + $scope.CurrentView);
            });
